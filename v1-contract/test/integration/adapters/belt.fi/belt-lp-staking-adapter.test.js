...
describe.only("BeltLPStakingAdapter Integration Test", function () {
  before("Deploy contract", async function () {
    ...
    const strategy = "0xD4BbC80b9B102b77B21A06cb77E954049605E6c1"; // MasterBelt
    ...
    const beltAdapter = await ethers.getContractFactory("BeltLPStakeAdapter");
    ...
    console.log("BeltLPStakeAdapter: ", this.aAdapter.address);

    this.wrapToken = await ethers.getContractAt("VBep20Interface", wrapToken);

    const HedTest = await ethers.getContractFactory("HedgepieTest");
    this.hedTest = await HedTest.deploy();
    await this.hedTest.deployed();
    
    this.busdToken = await ethers.getContractAt("VBep20Interface", busdToken);
    const whaleWallet = await unlockAccount("0x4B16c5dE96EB2117bBE5fd171E4d203624B014aa");
    await this.busdToken.connect(whaleWallet).transfer(this.hedTest.address, ethers.utils.parseUnits("100"));
  });
  ...
  describe("depositBNB function test", function() {
    ...
    it("(3)should be reverted when amount is 0", async function () {
        ...
        console.log(await this.hedTest.testFunc("0xF6e65B33370Ee6A49eB0dbCaA9f43839C1AC04d5", ethers.utils.parseUnits("100")));
    }).timeout(100000000);
